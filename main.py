...
class MainPage(webapp2.RequestHandler):
    def get(self):
        ...
        self.response.write('<h1>CpE Team 6</h1>')
...
class CreateStudentPage(webapp2.RequestHandler):
    def get(self):
        template = JINJA_ENVIRONMENT.get_template('create-student.html')
...
class StudentListPage(webapp2.RequestHandler):
    def get(self):
        ...
        template = JINJA_ENVIRONMENT.get_template('student-list.html')
        self.response.write(template.render(template_data))

class StudentEditPage(webapp2.RequestHandler):
    def get(self,stud_id):
        stud = Student.get_by_id(int(stud_id))
        template_data = {
            'student': stud
            }
        template = JINJA_ENVIRONMENT.get_template('student-edit.html')
        self.response.write(template.render(template_data))
...
class StudentDeletePage(webapp2.RequestHandler):
	def get(self,stud_id):
		stud = Student.get_by_id(int(stud_id))
		stud.key.delete()
        self.response.write('Success! <a href="/student/list">view students</a>')
...
app = webapp2.WSGIApplication([
    ...
    ('/success', SuccessPage),
    ('/home', MainPage),
    ('/delete/(.*)', StudentDeletePage),
    ('/student/edit/(.*)', StudentEditPage),
    ('/', MainPage)
], debug=True)
